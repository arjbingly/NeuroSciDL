...
# Dataset
DATA_DIR = Path('/gpfs/u/home/BDAE/BDAEbngl/scratch/data/eec')
ANNOTATIONS_DIR = DATA_DIR / 'annotations'
ANNOTATIONS_FILE = ANNOTATIONS_DIR / 'file_annotations_bal_v2.csv'
BATCH_SIZE = 128
...
datamodule = EEGDataModule(data_dir=DATA_DIR, annotation_dir=ANNOTATIONS_DIR, annotation_file=ANNOTATIONS_FILE.name,
                           batch_size=BATCH_SIZE, num_workers=NUM_WORKERS_DATAMODULE,
                           train_transform=gaussian_noise_transform)

# train model
metrics = [tm.classification.F1Score(task='binary', average='macro'), tm.classification.Accuracy(task='binary'),
           tm.classification.Precision(task='binary'), tm.classification.Recall(task='binary'), ]

model = EEGViT_pretrained(metrics=metrics)

trainer_args = {'max_epochs': MAX_EPOCHS, 'check_val_every_n_epoch': 1, }
...
if USE_MLFLOW:
    callbacks.extend([MlFlowModelSummary(), MlFlowHyperParams(trainer_args)])
    mlf_logger = pl.loggers.MLFlowLogger(log_model='all', experiment_name=EXPERIMENT_NAME, tracking_uri=TRACKING_URI)
